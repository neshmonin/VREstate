            ...
            foreach (var childNode in model_Node.ChildNodes)
            {
                ...
                if (elt.Name == "Location")
                {
                    foreach (var node in elt.ChildNodes)
                    {
                        ...
                        {
                            case "longitude":
                                try { m_lon0 = double.Parse(e.InnerText); }
                                catch (System.FormatException) { m_lon0 = 0.0; }
                                break;
                            case "latitude":
                                try { m_lat0 = double.Parse(e.InnerText); }
                                catch (System.FormatException) { m_lat0 = 0.0; }
                                break;
                            case "altitude":
                                try { m_alt0 = double.Parse(e.InnerText); }
                                catch (System.FormatException) { m_alt0 = 0.0; }
                                break;
                        }
                    }
                }
                ...
                if (elt.Name == "Orientation")
                {
                    foreach (var node in elt.ChildNodes)
                    {
                        ...
                        double val = 0.0;
                        try { val = double.Parse(e.InnerText); }
                        catch (System.FormatException) { val = 0.0; }

                        switch (e.Name)
                        {
                            case "heading":
                                m_heading_r = Angle.DegreesToRadians(val);
                                break;
                            case "tilt":
                                m_tilt_r = Angle.DegreesToRadians(val);
                                break;
                            case "roll":
                                m_roll_r = Angle.DegreesToRadians(val);
                                break;
                        }
                    }
                }
